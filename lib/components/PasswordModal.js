...
const PasswordModal = React.forwardRef((props, ref) => {
    const { close, onDone, clear = true, backdrop, header, title = '支付密码', tip = '', tipProps, inputViewProps = {}, keyboardProps = {}, } = props;
    ...
    return (<Modal animationType={'slide'} visible={visible} onRequestClose={hide} transparent={true}>
      <View style={[styles.container, { backgroundColor: backdrop ? 'rgba(0,0,0,0.3)' : 'transparent' }]}>
        <View style={{ backgroundColor: '#F5F5F5',paddingBottom:isX?24:0 }}>
          {header || <View style={styles.header}>
            <Text style={styles.title}>{title}</Text>
            <TouchableOpacity style={styles.close} onPress={closeEvent}>
